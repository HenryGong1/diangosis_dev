...
df -h | grep vblk_ufs0 | grep data
if [ $? != 0 ]; then
    echo "/data is not mounted"
    mount -t qnx6 -omntperms=0777,mntuid=3360,mntgid=3360 /dev/vblk_ufs0 /data
    if [ $? != 0 ]; then
        echo "cannot mount ufs0, format"
        (echo "y") | mkqnx6fs /dev/vblk_ufs0
        sleep 30
        mount -t qnx6 -omntperms=0777,mntuid=3360,mntgid=3360 /dev/vblk_ufs0 /data
        if [ $? != 0 ]; then
            echo "cannot mount ufs0 for nvidia_driveupdate"
            exit 1
        fi
    fi
else
